...
const getSupplier = async (req: Request, res: Response) => {
  try {
    ...
    if (!supplier) { ResponseService.json(res, 400, 'Supplier not found.'); return }
...
const deleteSupplier = async (req: Request, res: Response) => {
  ...
  if (!deletedSupplier) { ResponseService.json(res, 400, 'Supplier not found to be deleted.'); return }
