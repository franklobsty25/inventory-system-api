...
import { PaginateUserModel, UserDocument } from '../models/user.model';
import {
  ...
  try {
    ...
    if (value) {
      ...
      if (password !== repeatPassword) {
        ResponseService.json(res, 400, 'Password mismatch.');
        return;
      }
...
const login = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      ResponseService.json(res, 400, `User with ${email} not found.`);
      return;
    }
    ...
    if (!isValid) {
      ResponseService.json(res, 401, 'Password invalid.');
      return;
    }
...
const changePassword = async (req: CustomRequest, res: Response) => {
  try {
    ...
    if (!user) {
      ResponseService.json(res, 400, 'User not found for password changes.');
      return;
    }
    ...
    if (!isValid) {
      ResponseService.json(res, 403, 'Old password mismatch');
      return;
    }
...
const forgot = (req: Request, res: Response) => {};
...
const reset = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      ResponseService.json(res, 400, 'User not found for password reset.');
      return;
    }
...
const setOrChangeRole = async (req: Request, res: Response) => {
  try {
    ...
    if (value.role) {
      const isRole = Object.values(RolesEnum).includes(value.role);

      if (!isRole) return ResponseService.json(res, 400, 'Invalid role.');
    }
    ...
    if (!user) return ResponseService.json(res, 400, 'User not found.');
